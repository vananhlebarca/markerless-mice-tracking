...
def analyze_time_lapse_frames_va(config, directory, frametype='.jpg', shuffle=1, trainingsetindex=0, gputouse=None,
                              save_as_csv=True, rgb=False):
    """
    ...
    #dlc_cfg['num_outputs'] = cfg.get('num_outputs', 1)                   #-------------------------VA--------------
    dlc_cfg['num_outputs'] = cfg.get('num_outputs', 2)
        ...
        try:
            ...
            if nframes > 1:
                ...
                PredicteData, nframes, nx, ny = GetPosesofFrames(cfg, dlc_cfg, sess, inputs, outputs, directory,
                                                                 framelist, nframes, dlc_cfg['batch_size'], rgb)
                stop = time.time()
